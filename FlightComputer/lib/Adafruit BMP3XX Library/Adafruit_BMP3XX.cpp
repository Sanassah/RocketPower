...
/**************************************************************************/
/*!
    @brief  RocketPower fork: one-time switch to BMP3_MODE_NORMAL
    (continuous background sampling at the configured ODR), instead of the
    default forced (one-shot, trigger-and-wait) mode performReading() uses.

    Bench-measured (Teensy 4.1, this project): performReading() in forced
    mode was costing ~13-19ms per call, and instrumenting its 3 internal
    stages (settings write / op-mode transition / data read) showed the
    op-mode transition alone accounting for the majority of that --
    bmp3_set_op_mode() (bmp3.c) runs a hard-coded delay_us(5000, ...) sleep-
    transition plus its own register read/write every time forced mode is
    re-triggered, because forced mode always starts a brand new conversion
    from scratch rather than reading a value already being produced in the
    background. That's paid on EVERY call in forced mode, forever -- not a
    one-time cost.

    Normal mode configures the sensor to sample continuously at the ODR set
    via setOutputDataRate() and leaves it doing so; after this call,
    performReading() just fetches whatever the sensor most recently finished
    converting (bench-measured well under 1ms) instead of rewriting settings
    and re-triggering a conversion each time.

    NOTE: the oversampling/ODR combination passed in via setTemperature/
    PressureOversampling()+setOutputDataRate() before this call must leave
    enough time for a full conversion within one ODR period, or this fails
    with BMP3_E_INVALID_ODR_OSR_SETTINGS (Bosch's own
    validate_osr_and_odr_settings() in bmp3.c, checked only on entering
    normal mode -- forced mode never hits this, since it doesn't need to fit
    a conversion inside a periodic budget). Bench-confirmed the hard way:
    Barometer.cpp's oversampling settings are tuned to fit this.

    Call once, after the oversampling/filter/ODR setters, before the first
    performReading().

    @return True on success, False on failure
*/
/**************************************************************************/
bool Adafruit_BMP3XX::enableNormalMode(void) {
  g_i2c_dev = i2c_dev;
  g_spi_dev = spi_dev;
  int8_t rslt;
  ...
  _normalSensorComp = sensor_comp;
  _normalModeActive = true;
  return true;
}
...
bool Adafruit_BMP3XX::performReading(void) {
  ...
  if (_normalModeActive) {
    // RocketPower fork: already continuously sampling in the background
    // (see enableNormalMode()) -- just fetch the latest ready value below,
    // no settings-rewrite or mode-retrigger needed every call.
    sensor_comp = _normalSensorComp;

    // RocketPower fork: normal mode's background sampling writes fresh
    // conversion results into the DATA registers asynchronously to when we
    // happen to poll -- bmp3_get_sensor_data() below has no protection of
    // its own against reading mid-write (it's designed for forced mode's
    // trigger-then-read pattern, where a fresh conversion is always known
    // complete by the time it's called). Checking the DRDY status bits
    // first and treating "not ready yet" as skip-this-poll (not a failure
    // -- the caller just keeps its last good value) avoids a torn read:
    // some bytes from the old sample, some from a new one being written
    // concurrently. The sensor produces a fresh sample every ~20ms (the
    // configured ODR) and a caller can poll faster than that.
    uint8_t status = 0;
    rslt = bmp3_get_regs(BMP3_REG_SENS_STATUS, &status, 1, &the_sensor);
    if (rslt != BMP3_OK) {
      return false;
    }
    if (!(status & (BMP3_STATUS_DRDY_PRESS_MSK | BMP3_STATUS_DRDY_TEMP_MSK))) {
      return false;   // not ready yet -- not a real failure
    }
  } else {
